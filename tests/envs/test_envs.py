...
from lerobot.envs.configs import EnvConfig, LiberoEnv, MetaworldEnv
...
def test_make_env_pre_post_processors_xvla_libero_uses_libero_specific_steps():
    env_cfg = LiberoEnv(task="libero_goal")
    policy_cfg = XVLAConfig()

    preprocessor, postprocessor = make_env_pre_post_processors(env_cfg=env_cfg, policy_cfg=policy_cfg)
    ...
    assert [type(step).__name__ for step in postprocessor.steps] == ["XVLARotation6DToAxisAngleProcessorStep"]


def test_make_env_pre_post_processors_xvla_metaworld_is_identity():
    env_cfg = MetaworldEnv(task="medium")
    policy_cfg = XVLAConfig()

    preprocessor, postprocessor = make_env_pre_post_processors(env_cfg=env_cfg, policy_cfg=policy_cfg)

    assert preprocessor.steps == []
    assert postprocessor.steps == []
