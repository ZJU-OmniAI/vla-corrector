...
from .configuration_pi05_modified import PI05ModifiedConfig


class PI05PytorchModified(PI05Pytorch):
    """PI0.5 core model with optional safety-guided denoising updates."""
    ...
    def sample_actions(
        ...
        **kwargs,
        ...
        scheduled_steps = int((int(num_steps) + guidance_apply_every - 1) // guidance_apply_every)
...
class PI05ModifiedPolicy(PI05Policy):
    """PI0.5 policy wrapper with optional safety-guidance APIs.

    Original PI05 behavior remains unchanged when guidance is not requested.
    """

    config_class = PI05ModifiedConfig
    name = "pi05_modified"

    def __init__(self, config: PI05ModifiedConfig, **kwargs):
        PreTrainedPolicy.__init__(self, config)
        config.validate_features()
        self.config = config

        self.init_rtc_processor()
        self.model = PI05PytorchModified(config, rtc_processor=self.rtc_processor)
    ...
    def encode_siglip_only(self, batch: dict[str, Tensor]) -> np.ndarray:
        ...
        for image, mask in zip(images, img_masks, strict=False):
            ...
            if not use_block:
                continue
            token_blocks.append(self.model.paligemma_with_expert.embed_image(image))

        if not token_blocks:
            token_blocks.append(self.model.paligemma_with_expert.embed_image(images[0]))
    ...
    def predict_delta_z_only(self, z_t: np.ndarray, raw_action_t: np.ndarray) -> np.ndarray:
        if not self.model.safety_initialized or self.model.safety_predictor is None:
            raise RuntimeError("Safety predictor is not initialized in pi05_modified policy.")
        ...
        *,
        ...
        if guidance_replan and guidance_compare_baseline:
            ...
            compare_stats = self._compute_compare_stats(
                guided_raw[:, :, : self.config.output_features[ACTION].shape[0]],
                baseline_raw[:, :, : self.config.output_features[ACTION].shape[0]],
            )
