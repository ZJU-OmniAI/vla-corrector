...
class MetaworldEnv(gym.Env):
    ...
    def __init__(
        ...
        if max_path_length is not None:
            if int(max_path_length) <= 0:
                raise ValueError(f"max_path_length must be > 0, got {max_path_length}")
            self._env.max_path_length = int(max_path_length)
        self._max_episode_steps = int(self._env.max_path_length)
...
def create_metaworld_envs(
    ...
    Notes:
        ...
        - If `episode_length` is provided (>0), it overrides the underlying Meta-World `max_path_length`.
    """
    if env_cls is None or not callable(env_cls):
        raise ValueError("env_cls must be a callable that wraps a list of environment factory callables.")
    if not isinstance(n_envs, int) or n_envs <= 0:
        raise ValueError(f"n_envs must be a positive int; got {n_envs}.")
    if episode_length is not None and int(episode_length) <= 0:
        raise ValueError(f"episode_length must be a positive int when provided; got {episode_length}.")
    ...
    for group in task_groups:
        ...
        for tid, task_name in enumerate(tasks):
            ...
            fns = [
                (lambda tn=task_name: MetaworldEnv(task=tn, max_path_length=episode_length, **gym_kwargs))
