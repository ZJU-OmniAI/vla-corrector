...
def make_env_pre_post_processors(
    ...
    # XVLA currently needs environment-specific conversion only for LIBERO observations/actions.
    # For other environments (e.g. MetaWorld), keep env processors as identity and rely on
    # XVLA policy pre/post processors loaded from the checkpoint.
    if isinstance(policy_cfg, XVLAConfig) and (isinstance(env_cfg, LiberoEnv) or "libero" in env_cfg.type):
